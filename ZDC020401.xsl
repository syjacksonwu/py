...
<xsl:template match="TABELLE">
 <table border="3">
  ...
  <!-- 2. Zeile -->
  <xsl:choose>
   <xsl:when test="MODUS='F'"><tr/></xsl:when>
   <xsl:otherwise>
    <tr>
     <td colspan="3"/>
     ...
     <xsl:variable name="varSTELLEpos"><xsl:value-of select="position()"/></xsl:variable>
      <xsl:for-each select="ZDBYTE">
       <td  align = "center"><xsl:if test="$varSTELLEpos mod 2 = 1"><xsl:attribute name="bgcolor">#FFEEEE</xsl:attribute></xsl:if><xsl:value-of select="ZMSB"/></td>
      </xsl:for-each>
     </xsl:for-each>
    </tr>
    <tr>
     <td colspan = "3"></td><td align = "right">LSB</td>
     <xsl:for-each select="KOPF/ZDE">
      <xsl:variable name="varSTELLEpos"><xsl:value-of select="position()"/></xsl:variable>
      <xsl:for-each select="ZDBYTE">
       <td align = "center"><xsl:if test="$varSTELLEpos mod 2 = 1"><xsl:attribute name="bgcolor">#FFEEEE</xsl:attribute></xsl:if><xsl:value-of select="ZLSB"/></td>
      </xsl:for-each>
     </xsl:for-each>
    </tr>
   </xsl:otherwise>
  ...
  <tr>
   ...
   <xsl:for-each select="KOPF/ZDE">
    <xsl:variable name="varSTELLEpos"><xsl:value-of select="position()"/></xsl:variable>
    <xsl:for-each select="ZDBYTE">
     <td style="writing-mode:tb-rl"><xsl:if test="$varSTELLEpos mod 2 = 1"><xsl:attribute name="bgcolor">#FFEEEE</xsl:attribute></xsl:if><xsl:value-of select="ZBEZ"/></td>
    </xsl:for-each>
   </xsl:for-each>
  </tr>
     ...
     <xsl:if test="$varFAMpos mod 2 = 1"><xsl:attribute name="bgcolor">#EEFFEE</xsl:attribute></xsl:if>
      ...
      <xsl:for-each select="../../../KOPF/ZDE">
       ...
       <xsl:for-each select="ZDBYTE">
        ...
        <xsl:variable name="varVARNAME"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/VARNAME"/></xsl:variable>
        <xsl:variable name="varWERT"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/WERT"/></xsl:variable>
        <xsl:variable name="varDATEINAME"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/DATEINAME"/></xsl:variable>
        <xsl:variable name="varFEHLER"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/FEHLER"/></xsl:variable>
        <xsl:variable name="varUNGUELTIG"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/UNGUELTIG"/></xsl:variable>
        <xsl:variable name="varUEBERSPRINGEN"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/UEBERSPRINGEN"/></xsl:variable>
        <xsl:variable name="varDATEN-NAME"><xsl:value-of  select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/DATEN-NAME"/></xsl:variable>
        <xsl:choose>
        <xsl:when test="$varDATEINAME!=''">
    <td align = "center"><xsl:apply-templates select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/DATEINAME"/></td>
        </xsl:when>
        <xsl:when test="$varDATEN-NAME!=''">
    <td align = "center"><xsl:apply-templates select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/DATEN-NAME"/></td>
        </xsl:when>
        <xsl:when test="$varVARNAME!=''">
    <td align = "center"><xsl:apply-templates select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/VARNAME"/></td>
        </xsl:when>
        <xsl:when test="$varFEHLER!=''">
    <td align = "center"><xsl:apply-templates select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/FEHLER"/></td>
        </xsl:when>
        <xsl:when test="count(../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/FEHLER)>0">
    <td align = "center"><xsl:attribute name="bgcolor">#FF0000</xsl:attribute>ERROR</td>
        </xsl:when>
        <xsl:when test="$varUNGUELTIG!=''">
    <td align = "center"><xsl:apply-templates select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/UNGUELTIG"/></td>
        </xsl:when>
	<xsl:when test="count(../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/UNGUELTIG)>0">
    <td align = "center"><xsl:attribute name="bgcolor">#F0F000</xsl:attribute>N/A</td>
        </xsl:when>
        <xsl:when test="$varUEBERSPRINGEN!=''">
    <td align = "center"><xsl:apply-templates select="../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/UEBERSPRINGEN"/></td>
        </xsl:when>
        <xsl:when test="count(../../../TAB/FAM[FAMNR=$varPRNFAMILIENUMMER and position()=$varFAMpos]/TEGUE[PRNR=$varPRNNUMMER]/KNOTEN[STELLE=$varStelle and LSB=$varLSB]/UEBERSPRINGEN)>0">
    <td align = "center"><xsl:attribute name="bgcolor">#F0F000</xsl:attribute>N/A</td>
        </xsl:when>
